...
function runScenario(scenarioDir, opts, root) {
    ...
    const r = spawnSync("npx", ["promptfoo", "eval", "--no-cache", "--no-progress-bar", "-j", process.env.EVALS_CONCURRENCY ?? "4", "-c", configPath, "-o", resultPath], 
...
function discoverScenarios(root) {
    ...
    if (fs.existsSync(cliDir)) {
        for (const e of fs.readdirSync(cliDir, { withFileTypes: true })) {
            if (e.isDirectory())
                roots.push(path.join(cliDir, e.name, "skills"));
        }
    }
    const found = [];
    for (const dir of roots) {
        ...
        for (const skill of fs.readdirSync(dir, { withFileTypes: true })) {
            ...
            if (!skill.isDirectory() || !fs.existsSync(evalsDir))
                continue;
...
function cmdSweep(argv) {
    ...
    if (positional.length > 0)
        fail("usage: skillcheck sweep [--root DIR] [--all]");
...
export function reduceResults(dir, allowMixed) {
    const entries = [];
    const skipped = [];
    const shas = new Set();
...
function cmdSummarize(argv) {
    const { positional, flags } = parseArgs(argv);
    if (positional.length > 0)
        fail("usage: skillcheck summarize [--root DIR] [--allow-mixed]");
...
function cmdLint(argv) {
    const { positional, flags } = parseArgs(argv);
    if (positional.length > 1)
        fail("usage: skillcheck lint [<root>] [--root DIR]");
...
if (isMainModule()) {
    ...
    try {
        if (cmd === "run")
            ...
            fail("usage: skillcheck <lint|run|sweep|summarize> ...");
    }
    catch (err) {
